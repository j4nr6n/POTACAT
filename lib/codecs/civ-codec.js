...
class CivCodec extends EventEmitter {
  ...
  setFilterWidth(hz) {
    // IC-7300 etc: use passband width command
    // CI-V doesn't have a direct filter width command — use FIL select
    // For now, this is model-specific and may need expansion
    // Most Icom radios: cmd 0x06 with filter byte 1/2/3
    if (hz <= 500) {
      this._sendCmd(0x06, null, [this._lastModeByte || 0x03, 0x03]); // narrow
    } else if (hz <= 1800) {
      this._sendCmd(0x06, null, [this._lastModeByte || 0x03, 0x02]); // medium
    } else {
      this._sendCmd(0x06, null, [this._lastModeByte || 0x01, 0x01]); // wide
    }
  }
