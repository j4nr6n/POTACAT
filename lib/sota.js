...
const SPOT_URL = 'https://api2.sota.org.uk/api/spots/200/all';
...
async function loadAssociations() {
  ...
  try {
    const data = await httpsGetJson('https://api2.sota.org.uk/api/associations');
    for (const a of data) {
...
class SotaUploader extends EventEmitter {
  ...
  async _login() {
    ...
    const body = `client_id=${SOTA_CLIENT_ID}&grant_type=password&username=${encodeURIComponent(this._username)}&password=${encodeURIComponent(this._password)}`;
  ...
  async _refresh() {
    ...
    const body = `client_id=${SOTA_CLIENT_ID}&grant_type=refresh_token&refresh_token=${encodeURIComponent(this.refreshToken)}`;
    const res = await httpsPost(SSO_HOST, SSO_TOKEN_PATH, body);
    if (res.status !== 200 || !res.json || !res.json.access_token) {
      this.refreshToken = null;
      return this._login();
