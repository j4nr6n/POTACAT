...
class CloudSyncClient extends EventEmitter {
  ...
  configure(opts) {
    ...
    if (opts.accessToken) this._accessToken = opts.accessToken;
  ...
  async _ensureAuth() {
    ...
    // Otherwise try to refresh (covers: expired token, _tokenExpiry never set, no access token but have refresh)
