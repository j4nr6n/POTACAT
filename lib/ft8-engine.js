...
class Ft8Engine extends EventEmitter {
  ...
  async encodeMessage(text, freq) {
    ...
    if (this._mode === 'FT4') return null; // FT4 encode not implemented
  ...
  _onCycleBoundary() {
    ...
    // FT4 decode is not yet implemented — skip decode, still allow RX cycle events
    if (this._mode === 'FT4') {
      this._audioBuffer = new Float32Array(FT8_SAMPLES);
      this._audioOffset = 0;
      this.emit('decode', { cycle: this._cycleNumber, results: [] });
      return;
    }
